...
async function fetchInvestorRecords(): Promise<Map<string, ZohoInvestorRecord>> {
  ...
  while (more) {
    ...
    for (const rec of data.data ?? []) {
      ...
      if (rec.Secondary_Email) {
        const sk = rec.Secondary_Email.toLowerCase()
        if (sk !== key && !secondaryIndex.has(sk)) secondaryIndex.set(sk, source)
      }
    }
  ...
  secondaryIndexCache = { data: secondaryIndex, expiresAt: Date.now() + SOURCE_CACHE_TTL_MS }
  return map
}
 ...
 * Includes secondary/alternate emails (e.g. a family member listed on the
 * same Zoho record) so a myQode email that only appears as a Zoho secondary
 * email still resolves to a source. Primary emails take precedence.
 */
export async function getInvestorSourceMap(): Promise<Map<string, string>> {
  const records = await fetchInvestorRecords()
  const map = new Map<string, string>()
  // Secondary emails first, so primaries overwrite on any collision.
  if (secondaryIndexCache) for (const [email, source] of secondaryIndexCache.data) map.set(email, source)
