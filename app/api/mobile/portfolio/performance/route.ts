...
// CAGR return % — uses fixed year count from the period (not actual date diff)
...
// Since-inception return: CAGR if >= 1 year, absolute otherwise
...
// Find closest nav on or before a cutoff date in an ASC-sorted array
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // Max drawdown (most negative value in DB column = largest drawdown magnitude)
      ...
      const benchResult = await db2.query(
        `SELECT date, nav
         ...
         WHERE indices = $1
           ...
           AND date <= $3
         ORDER BY date DESC`,
        [benchmarkIndex, inceptionDate, benchEndDate]
      )
