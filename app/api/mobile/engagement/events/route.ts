...
// Returns list of event PDFs/materials from S3 with signed URLs, sorted newest first.
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const items = await Promise.all(
      objects.map(async (obj) => {
        ...
        return {
          ...
          section,
          url,
          type: ext === 'pdf' ? 'pdf' as const : 'file' as const,
          size: obj.Size ?? 0,
    ...
    console.error('[mobile/engagement/events]', err)
